...
func TestUTF8UTF8Validator_Decode(t *testing.T) {
	...
	_, err = dec.Decode(in)
	if err == nil {
		t.Fatalf("wanted an error in Decode, but got no error")
	}
	if DBaseError(err.Error()) != ERROR_INVALID_ENCODING {
		t.Fatalf("wanted error %s, have %s", ERROR_INVALID_ENCODING, err)
	}
